...
# -*- coding: utf-8 -*-
"""
Created on Wed Mar  4 10:19:01 2020
...
plt.figure(3)
plt.scatter(CCA_learned[1,:],y_test[1,:])
...
indexes = np.asarray(comparison2.index[comparison2['actual_class']=='cat'].tolist())
rscore_cat = np.zeros((9, 9))
score = np.corrcoef(BottleNeck[indexes,:])
np.mean(score[:,1])
indexes2 = np.asarray(comparison2.index[comparison2['actual_class']== 'dog' ].tolist())
indexes21 = np.asarray(comparison2.index[comparison2['actual_class']== 'hammer' ].tolist())
indexes3 = np.append(indexes, indexes2)
indexes31 = np.append(indexes, indexes21)
indexes = np.zeros((12, len(classes)))
#indexes.fill(np.nan)
i=0
for categ in classes:
    index2 = np.asarray(comparison2.index[comparison2['actual_class']== categ ].tolist())
    o = len(index2)
    indexes[:o,i] = index2
    i+=1

rscore_classes_mean = np.zeros(len(classes))
for i in range(len(classes)):
    newind = np.delete(indexes[:,i], np.where(indexes[:,i]==0)).astype(int) 
    rscore_classes = np.corrcoef(BottleNeck[newind,])
    rscore_classes_mean[i]= np.mean(rscore_classes[1:,0])
sum(rscore_classes_mean>0.2)
